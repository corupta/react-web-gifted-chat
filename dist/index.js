...
function Send(_ref) {
  ...
  if (alwaysShowSend || text.trim().length > 0) {
    return React__default.createElement(TouchableOpacity, {
      ...
      style: [styles$c.container, containerStyle],
      onPress: function onPress() {
        onSend({
          text: text.trim()
        }, true);
...
function (_React$Component) {
  ...
  _createClass(MessageContainer, [{
    key: "shouldComponentUpdate",
    value: function shouldComponentUpdate(nextProps) {
      var next = nextProps.messages;
      var current = this.props.messages;
      return next.length !== current.length || next.extraData !== current.extraData || next.loadEarlier !== current.loadEarlier;
    }
  }, {
    key: "scrollTo",
...
function (_React$Component) {
  ...
  _createClass(GiftedChat, [{
    ...
    value: function onInputSizeChanged(size) {
      var newComposerHeight = Math.max(this.props.minComposerHeight, Math.min(this.props.maxComposerHeight, size.height));
      var newMessagesContainerHeight = this.getMessagesContainerHeightWithKeyboard(newComposerHeight);
      this.setState({
        composerHeight: newComposerHeight,
        messagesContainerHeight: this.prepareMessagesContainerHeight(newMessagesContainerHeight)
